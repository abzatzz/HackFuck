    ...
    try:
        for time_frame in creds.time_frame_list:
            ...
            if time_frame == '5m' or time_frame == '15m':
                high_limit = creds.high_limit_m
                low_limit = creds.low_limit_m
            elif time_frame == '1h':
                high_limit = creds.high_limit_1h
                low_limit = creds.low_limit_1h
            elif time_frame == '4h':
                high_limit = creds.high_limit_4h
                low_limit = creds.low_limit_4h
                ...
                if current_voluve > creds.min_volume and str(el_tickers['symbol']).endswith('USDT') \
                    ...
                    for i_up in range(l_up):
                        ...
                        for el_up in up_fractals[i_up:l_up]:
                            if count_up == 0:
                                ...
                                # print('***', pd.to_datetime(el_up[open_time], unit='ms'), float(el_up[high]), '(', max_level_up, ')', count_up)

                            elif float(el_up[high]) <= base_level_up * 1.002 and  float(el_up[high]) >= base_level_up * 0.997:
                                possible_high = base_level_up * 1.002

                                count_up += 1
                                print('Time - ', str(pd.to_datetime(el_up[open_time], unit='ms')) , symbol_to_work, time_frame, 'Series counter', 'Current -', count_up, float(el_up[high]), 'Base -', base_level_up, 'Maximum -', possible_high )
                                if float(el_up[high]) > max_level_up:
                                    ...
                                    if count_up >= 3 and max_level_up > max_last_up \
                                        ...
                                        data_to_write = 'Level' + ';' + str(count_global) + ';' \
                                                        + 'Ticker' + ';' + symbol_to_work + ';' + 'Trend' + ';' \
                                                        + 'HIGH' + ';' + 'TimeFrame' + ';' + time_frame + ';' + 'FirstLevel' \
                                                        + ';' + f_high_1 + '--' + str(first_level_high) + ';' + 'SecondLevel' + ';' \
                                                        + f_high_2 + '--' + str(second_level_high) + ';' + 'ThirdLevel' + ';' + f_high_3 \
                                                        + '--' + str(third_level_high) + ';' + 'MaxLevel' \
                                                        + ';' + str(max_level_up)
                                        ...
                                        fourth_take = False
                                        fifth_take = False
                                        current_level_up = max_level_up
                                        # print(symbol_to_work, '--',  time_frame, '--', max_level_up, '--', f_high_3)
                                        for masi  in mas[el_up[index_kline] +1:len(mas)]:
                                            if peresekli :
                                                if float(masi[close_kline]) < float(masi[open_kline]) and (float(current_level_up - float(masi[close_kline])))/current_level_up*100 < 0.44:
                                                    stop_loss_level = current_level_up * 0.9956
                                                    stop_loss_time = str(pd.to_datetime(masi[close_time], unit='ms'))
                                                    print(symbol_to_work, '--',  time_frame, stop_loss_time , '***',  'stop loss', '--', first_take, second_take, third_take, fourth_take, fifth_take, stop_loss_level)
                                                    break
                                            if max_level_up < float(masi[high]):
                                                peresekli = True
                                                if (float(masi[high]) - max_level_up)/max_level_up*100 > 0.54:
                                                    first_take = True
                                                    current_level_up = max_level_up * 1.0054
                                                    if (float(masi[high]) - max_level_up)/max_level_up*100 > 1.04:
                                                        second_take = True
                                                        current_level_up = max_level_up * 1.0104
                                                        if (float(masi[high]) - max_level_up)/max_level_up*100 > 1.54:
                                                            third_take = True
                                                            current_level_up = max_level_up * 1.0154
                                                            if (float(masi[high]) - max_level_up)/max_level_up*100 > 2.04:
                                                                fourth_take = True
                                                                current_level_up = max_level_up * 1.0204
                                                                if (float(masi[high]) - max_level_up)/max_level_up*100 > 3.04:
                                                                    fifth_take = True
                                                                    current_level_up = max_level_up * 1.0304
                                                                    print('full succes', '-----', current_level_up)
                                                                    break
                                                # perebiti = (float(masi[high]) - max_level_up)/max_level_up*100
                                                # if perebiti > 1:
                                                #     counter_2 += 1
                                                #     print(symbol_to_work, '--', \
                                                #           time_frame, '--', third_level_high, '--', f_high_3, '--', masi[high], '--',
                                                #           str(pd.to_datetime(masi[open_time], unit='ms')), '--', perebiti, '(', float(masi[volume]) * float(masi[high]), ')')
                                                # counter_1 += 1
                                                # print(counter_1, counter_2)
                                                # break
                                        # print('Level to bet highs - ', pd.to_datetime(el_up[open_time], unit='ms'),first_level_high, second_level_high, third_level_high, '>>>', max_level_up, '<<<', count_up)
                            elif float(el_up[high]) > base_level_up * 1.02:
                                count_up = 0
                                first_level_high = 0
                                second_level_high = 0
                                third_level_high = 0
                        ...
                        #     or (first_level_high < second_level_high and second_level_high == third_level_high)):
                        #     data_to_write = 'Level' + ';' + str(count_global) + ';' \
                        #                     + 'Ticker' + ';' + symbol_to_work + ';' + 'Trend' + ';' + 'HIGH' + ';' + 'TimeFrame' + ';' \
                    ...
                    while counter_mas < len(mas)-creds.fractal_periods:
                        ...
                        if is_low_fractal:
                            ...
                            down_fractals.append(mas[counter_mas])
                    ...
                    for i_down in range(l_down):
                        ...
                        for el_down in down_fractals[i_down:l_down]:
                            if count_down == 0:
                                ...
                                # print('***', pd.to_datetime(el_down[open_time], unit='ms'), float(el_down[low]), '(', max_level_down, ')', count_down)
                            else:
                                if float(el_down[low]) >= base_level_down - (base_level_down * high_limit / 100) \
                                    and  float(el_down[low]) <= base_level_down + (base_level_down * low_limit / 100):
                                    count_down += 1
                                    ...
                                    if count_down >= 3:
                                        ...
                                        # print('Level to bet  low - ', pd.to_datetime(el_down[open_time], unit='ms'), first_level_low, second_level_low, third_level_low, '>>>', max_level_down, '<<<', count_down)
                                elif float(el_down[low]) < base_level_down - ( base_level_down * low_limit/100 ):
                                    count_down = 0
                                    first_level_low = 0
                                    second_level_low = 0
                                    third_level_low = 0
                        ...
                        if count_down >= 3 and max_level_down < max_last_down \
                            ...
                            data_to_write = 'Level' + ';' + str(count_global) + ';' \
                                            + 'Ticker' + ';' + symbol_to_work + ';' + 'Trend' + ';' + 'LOW' + ';' + 'TimeFrame' + ';' \
